...
class PlayState extends MusicBeatState
{
	...
	private var pauseChartOnBeginning:Bool = false;
	...
	var onEndDialogue:Bool = false;
	var dialoguePart:Int = 0;
	...
	override public function create()
	{
		...
		switch(songLowercase)
		{
			...
			case 'senpai' | 'roses' | 'thorns' | 'tost' | 'burnt':
				...
				case 'tost':
					{
						defaultCamZoom = 0.9;
						curStage = 'stage';
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('Mod/Bg/bg1/stageback'));
						bg.antialiasing = FlxG.save.data.antialiasing;
						bg.scrollFactor.set(0.9, 0.9);
						bg.active = false;
						add(bg);

						var stageFront:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('Mod/Bg/bg1/stagefront'));
						...
						var stageCurtains:FlxSprite = new FlxSprite(-500, -300).loadGraphic(Paths.image('Mod/Bg/bg1/stagecurtains'));
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.ENTER && onEndDialogue)
		{
		...
		if (controls.PAUSE && startedCountdown && canPause && !cannotDie && !onEndDialogue)
		{
	...
	override function beatHit()
	{
		...
		if (curSong == 'Burnt' && curStep == 1380)
		{
			...
			myText = new FlxText(0, 50, 500); // x, y, width
			myText.screenCenter(X);
			//myText.screenCenter(Y);
			myText.text = "Dear player, \n \nYou've reached the end of the demo, congratulations! Don't you worry just yet, the full mod will come out very soon! Make sure to check out my Twitter for more updates! (@leoFriends2)\n \nMassive thanks to Mr. Pizza for helping with the sprites, NickAnimations3711 for programming, and Smolboi Oswald for helping me put Girlfriend in! \n \nThank you for playing, and I hope to see you when the mod is fully out!\n \n Press the ENTER key to return to menu
			";
			myText.setFormat('Pixel Arial 11 Bold', 20, FlxColor.WHITE, LEFT);
			myText.setBorderStyle(OUTLINE, FlxColor.RED, 1);
			myText.cameras = [camHUD2];
